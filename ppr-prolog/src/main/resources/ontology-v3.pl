...
i_has_relation(S,T,R,L) :- i_rdfs_sub_property_of(Z,R) , not(visited(Z,L)), i_has_relation(S,T,Z,[Z|L]) .
...
i_has_policy(T,P,L,LP) :- i_has_relation(S,T,R), not(visited(S,L)), i_propagates(R,P), not(visited(P,LP)), i_has_policy(S,P,[S|L],[P|LP]) .
